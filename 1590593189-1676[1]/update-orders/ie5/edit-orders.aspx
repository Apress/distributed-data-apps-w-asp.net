...
// load a list of orders for this customer from server
  ...
  if (strCustID == '') {
    ...
    document.all('lblStatus').innerHTML =
      ...
      + '<a href="../../customer-orders/ie5/default.aspx">'
    ...
    var strURL = 'order-data.aspx?list=customers&customerid=' + strCustID;
...
// value 4 indicates loading complete
  ...
  if (objXMLData.readyState == 4) {
    ...
    if (objXMLData.parseError.errorCode != 0) {
      ...
      //load list of shippers from server
      if (loadShippersList()) {
        ...
        document.all('lblStatus').innerHTML =
          '<b>* ERROR</b> - could not load product details.';
      }
      else
        document.all('lblStatus').innerHTML =
          '<b>* ERROR</b> - could not load shipper details.';
    }
  }
}
...
// load a list of shippers from server

  // create a new parser object instance and load data
  objXMLShippers = new ActiveXObject('MSXML2.FreeThreadedDOMDocument');
  objXMLShippers.async = false;
  if (objXMLShippers.load('order-data.aspx?list=shippers')) {
    ...
    for (var intLoop = 0; intLoop < objNodeSet.length; intLoop++) {
      ...
      objViaList.options.length += 1;
      objViaList.options[objViaList.options.length - 1].value = strShipID;
      objViaList.options[objViaList.options.length - 1].text = strShipName;
    }
    return true;
  }
}
...
// load a list of products from server

  // create a new parser object instance and load data
  objXMLProducts = new ActiveXObject('MSXML2.FreeThreadedDOMDocument');
  objXMLProducts.async = false;
  if (objXMLProducts.load('order-data.aspx?list=products')) {
    ...
    var objViaList = document.all('selProducts');
    ...
    for (var intLoop = 0; intLoop < objNodeSet.length; intLoop++) {
      ...
      objViaList.options.length += 1;
      objViaList.options[objViaList.options.length - 1].value = strProductID;
      objViaList.options[objViaList.options.length - 1].text = strProductName;
    }
    return true;
  }
}
...
// display the list of orders in the left-hand table
  ...
  if (strResult.length > 0) {
    ...
    var msg = 'Click an <b>Order ID</b> in the grid above<br />'
            ...
            + 'or <a href="../../customer-orders/ie5/default.aspx">'
            + '<b>select a different customer</b></a>.'
  }
  else
...
function postToServer() {
  // send XML DiffGram to server to update database
  ...
  // insert XML and CustomerID into hidden controls and submit form
  document.all('hidCustID').value = strCustID;
  document.all('hidPostXML').value = objXMLData.xml;
  document.all('frmPostXML').submit();
}
...
function checkIfDirty() {
  ...
  if (blnIsDirty) {
    ...
    if (confirm(strConfirm)) postToServer();
  }
}
...
<body link="#0000ff" alink="#0000ff" vlink="#0000ff" onload="loadOrderList()" onunload="checkIfDirty()">

<div class="heading">Edit Customer Orders</div>

<div align="right" class="cite">
[<a href="../../global/viewsource.aspx?compsrc=order-data.aspx">view page source</a>]<br />
...
<table border="0" cellpadding="20"><tr><td valign="top" bgcolor="#fffacd">
  ...
  <div id="divSaveChanges" align="center" style="padding=10">
    <input type="button" id="btnSaveChanges" disabled="true" value="Save Changes" onclick="postToServer()" />
  </div>
  <span>
...
<form id="frmPostXml" action="reconcile-changes.aspx" method="post">
