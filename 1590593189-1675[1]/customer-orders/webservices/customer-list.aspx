...
// runs when the page is first opened to load the customer list
  ...
  // we want a list of all customers, so empty strings are provided
  // for the parameters CustID and CustName parameters in this case
  var iCallID = htcWService.CustData.callService(dataLoaded, "GetCustomers", "", "");
}
...
// runs once the Web Service has loaded the data from the server
  ...
  if(objResult.error) {
    ...
    document.all('lblStatus').innerHTML = 'Loading customer data '
  ...
  if (objXMLData.readyState == 4) {
    ...
    if (objXMLData.parseError.errorCode != 0)
      ...
      document.all('lblStatus').innerHTML =
        '<b>* ERROR</b> - could not parse customer list.';
...
// customer list as a string containing the HTML to display
  ...
  var strStyle = '<?xml version="1.0" ?>\n'
    ...
    + '       <a>\n'
    + '         <xsl:attribute name="href">\n'
    + '          ../../update-orders/webservices/edit-orders.aspx?customerid=<xsl:value-of select="CustomerID" />\n'
    + '         </xsl:attribute>\n'
    + '         Edit Orders\n'
    + '       </a>\n'
    + '      </td>\n'
...
<div class="heading" style="behavior:url(../../global/webservice.htc)" id="htcWService">
  View Customer Orders - Select Customer
</div>
...
<table border="0" cellpadding="20"><tr><td valign="top" bgcolor="#fffacd">
  ...
  <span id="lblStatus">Locating customer data Web<br />Service - please wait ...</span>
